...
const items=[['/','Home',LayoutDashboard],['/trends','Discover',Flame],['/studio','Create',Sparkles],['/analytics','Results',Activity],['/competitors','Intelligence',Users],['/calendar','Plan',CalendarDays],['/settings','Settings',Settings]] as const
  ...
  return <main className="min-h-screen bg-[#f4f3ee] text-[#111113]">
    <header className="sticky top-0 z-40 border-b border-black/[.08] bg-[#f4f3ee]/95 backdrop-blur-sm">
      <div className="mx-auto flex h-[74px] max-w-[1480px] items-center justify-between px-5 lg:px-9">
        ...
        <button aria-label={menuOpen?'Close menu':'Open menu'} aria-expanded={menuOpen} onClick={()=>setMenuOpen(v=>!v)} className={`vyral-menu-button ${menuOpen?'is-open':''}`}><span className="vyral-menu-line vyral-menu-line-top"/><span className="vyral-menu-line vyral-menu-line-bottom"/></button>
      </div>
    </header>
