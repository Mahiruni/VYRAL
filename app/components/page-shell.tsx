  ...
  return <main className="min-h-screen bg-[#f4f3ee] text-[#111113]">
    <header className="sticky top-0 z-40 border-b border-black/[.08] bg-[#f4f3ee]/95 backdrop-blur-sm">
      <div className="mx-auto flex h-[74px] max-w-[1480px] items-center justify-between px-5 lg:px-9">
        <Link href="/" onClick={()=>setMenuOpen(false)} className="group flex items-center gap-3 press"><div className="grid h-9 w-9 place-items-center bg-[#111113] text-white transition-transform duration-300 group-hover:-rotate-3"><Zap size={16} fill="currentColor"/></div><div className="font-display text-[26px] font-semibold leading-none tracking-[-.045em]">VYRAL</div></Link>
        <nav className="hidden items-center gap-5 lg:flex">{items.map(([href,label,Icon])=><Link key={href} href={href} className="group flex items-center gap-2 px-2 py-2 text-[11px] font-semibold text-black/45 transition hover:text-black"><Icon size={13} className="text-black/25 transition group-hover:text-[#ff3d55]"/>{label}</Link>)}</nav>
        <div className="flex items-center gap-2"><Link href="/settings" aria-label="Settings" className="hidden h-9 w-9 place-items-center border border-black/[.08] bg-white text-black/45 transition hover:bg-black hover:text-white press sm:grid"><Settings size={15}/></Link><button aria-label={menuOpen?'Close menu':'Open menu'} aria-expanded={menuOpen} onClick={()=>setMenuOpen(v=>!v)} className={`vyral-menu-button ${menuOpen?'is-open':''}`}><span className="vyral-menu-line vyral-menu-line-top"/><span className="vyral-menu-line vyral-menu-line-bottom"/><span className="vyral-menu-label">{menuOpen?'Close':'Menu'}</span></button></div>
      </div>
      <div className="mx-auto flex max-w-[1480px] items-center gap-1 overflow-x-auto border-t border-black/[.05] px-5 py-2 lg:hidden"><Link href="/" onClick={()=>setMenuOpen(false)} className="bg-black px-3 py-1.5 text-[10px] font-bold text-white">Home</Link>{items.slice(1).map(([href,label])=><Link key={href} href={href} onClick={()=>setMenuOpen(false)} className="whitespace-nowrap px-3 py-1.5 text-[10px] font-semibold text-black/50 hover:bg-black/[.05]">{label}</Link>)}</div>
    </header>

    <div className={`vyral-menu-overlay ${menuOpen?'is-open':''}`} aria-hidden={!menuOpen}><div className="vyral-menu-glow"/><div className="mx-auto flex min-h-screen max-w-[1480px] flex-col px-6 pb-10 pt-[112px] lg:px-12 lg:pt-[132px]"><div className="mb-10 flex items-end justify-between border-b border-black/[.1] pb-5"><div><h2 className="font-display text-4xl font-semibold tracking-tight lg:text-6xl">Where do you want to go?</h2></div><span className="hidden font-mono text-[9px] uppercase tracking-[.16em] text-black/35 sm:block">ESC to close</span></div><nav className="grid gap-1 sm:grid-cols-2">{items.map(([href,label,Icon],i)=><Link key={href} href={href} onClick={()=>setMenuOpen(false)} className="vyral-menu-item" style={{'--menu-delay':`${i*55}ms`} as CSSProperties}><span className="vyral-menu-index">0{i+1}</span><span className="vyral-menu-icon"><Icon size={17}/></span><span className="vyral-menu-title">{label}</span><ArrowUpRight className="vyral-menu-arrow" size={22}/></Link>)}</nav><div className="mt-auto flex flex-col gap-5 border-t border-black/[.1] pt-7 sm:flex-row sm:items-center sm:justify-between"><p className="max-w-md text-sm leading-6 text-black/45">Find the idea. Make the move. Learn what matters.</p><Link href="/studio" onClick={()=>setMenuOpen(false)} className="inline-flex w-fit items-center gap-2 bg-[#111113] px-5 py-3 text-[11px] font-bold text-white transition hover:bg-[#ff3d55] press">Create a video <ArrowUpRight size={14}/></Link></div></div></div>
